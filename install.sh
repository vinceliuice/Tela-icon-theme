...
if [ ${UID} -eq 0 ]; then
  readonly DEST_DIR="/usr/share/icons"
else
  readonly DEST_DIR="${HOME}/.local/share/icons"
fi
...
install_theme() {
  ...
  install -m644 "${SRC_DIR}/COPYING"                                           "${THEME_DIR}"
  install -m644 "${SRC_DIR}/AUTHORS"                                           "${THEME_DIR}"
  ...
  if [[ ${bright} != "-Dark" ]]; then
    ...
    ln -sr "${THEME_DIR%-Dark}/scalable"                                       "${THEME_DIR}/scalable"
    ln -sr "${THEME_DIR%-Dark}/symbolic"                                       "${THEME_DIR}/symbolic"
    ln -sr "${THEME_DIR%-Dark}/32"                                             "${THEME_DIR}/32"
    ln -sr "${THEME_DIR%-Dark}/16/apps"                                        "${THEME_DIR}/16/apps"
    ln -sr "${THEME_DIR%-Dark}/16/mimetypes"                                   "${THEME_DIR}/16/mimetypes"
    ln -sr "${THEME_DIR%-Dark}/16/panel"                                       "${THEME_DIR}/16/panel"
    ln -sr "${THEME_DIR%-Dark}/16/status"                                      "${THEME_DIR}/16/status"
    ln -sr "${THEME_DIR%-Dark}/22/emblems"                                     "${THEME_DIR}/22/emblems"
    ln -sr "${THEME_DIR%-Dark}/22/mimetypes"                                   "${THEME_DIR}/22/mimetypes"
    ln -sr "${THEME_DIR%-Dark}/22/panel"                                       "${THEME_DIR}/22/panel"
    ln -sr "${THEME_DIR%-Dark}/24/animations"                                  "${THEME_DIR}/24/animations"
    ln -sr "${THEME_DIR%-Dark}/24/panel"                                       "${THEME_DIR}/24/panel"
  fi
